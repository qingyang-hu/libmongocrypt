...
static void
_test_createdatakey_with_credentials (_mongocrypt_tester_t *tester)
{
   mongocrypt_t *crypt;
   ...
   /* Assert first CTX_NEED_KMS state requests access token. */
   {
      ...
      ASSERT_OK (mongocrypt_kms_ctx_feed (
                    kms, TEST_FILE ("./test/data/gcp-auth/oauth-response.txt")),
                 kms);
...
static void
_test_encrypt_with_credentials (_mongocrypt_tester_t *tester)
{
   mongocrypt_t *crypt;
   mongocrypt_ctx_t *ctx;
   mongocrypt_kms_ctx_t *kms;
...
_test_createdatakey_with_wrong_kms_provider (_mongocrypt_tester_t *tester)
{
   ...
   ASSERT_FAILS (mongocrypt_ctx_datakey_init (ctx),
                 ctx,
                 "TODO: expect error message because the 'azure' KMS provider "
                 "is not set in mongocrypt_setopt_kms_providers");
