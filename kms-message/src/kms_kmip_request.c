...
enum TAG_TYPE {
    ...
    TAG_CertificateIssuerAlternativeName = 0x420016, //(deprecatedasofvers=ion1.1),
    TAG_CertificateIssuerDistinguishedName = 0x420017, //(deprecatedasofvers=ion1.1),
    ...
    TAG_CertificateSubjectAlternativeName = 0x42001B, //(deprecatedasofvers=ion1.1),
    TAG_CertificateSubjectDistinguishedName = 0x42001C, //(deprecatedasofvers=ion1.1),
...
write_i64_datetime (kmip_writer_t *writer, int32_t tag, int64_t value)
{
   ...
   void begin_struct(kmip_writer_t *writer, int32_t tag) {
       write_tag_enum(writer, tag);
    ...
    void close_struct(kmip_writer_t *writer) {
        // println!(" close inner");
...
// Dump into payload
   if (!kms_request_append_payload (req, writer->buffer->str, writer->buffer->len)) {
      goto done;
   }
